...
def generate_summary():
    ...
    h.append('    th:first-child { text-align: left; min-width: 220px; }')
    ...
    h.append('                     display: flex; align-items: center; justify-content: center;')
    h.append('                     margin: 0 auto; }')
    h.append('  </style>')
    h.append('</head>')
    h.append('<body>')
    ...
    h.append('  </div>')
    h.append('  <script>')
    h.append('    document.querySelector("table").addEventListener("click", function(e) {')
    h.append('      var row = e.target.closest("tr");')
    h.append('      if (row && !row.querySelector("th")) row.classList.toggle("highlighted");')
    h.append('    });')
    h.append('  </script>')
    h.append('</body>')
    h.append('</html>')
