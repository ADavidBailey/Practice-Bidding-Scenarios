...
def generate_summary():
    ...
    h.append('      });')
    h.append('      table.querySelectorAll(".diff").forEach(function(c) {')
    h.append('        c.classList.remove("diff");')
    h.append('      });')
    h.append('      hCol = -1;')
    h.append('    }')
    ...
    h.append('        var ref = rows[i].cells[col];')
    ...
    h.append('          if (j === col) continue;')
    h.append('          var cell = rows[i].cells[j];')
    h.append('          var cellVal = cell.getAttribute("data-v");')
    h.append('          if (cellVal !== null && cellVal !== refVal) {')
    h.append('            cell.classList.add("diff");')
    h.append('          }')
    h.append('        }')
    h.append('      }')
    h.append('    }')
